...
Download the latest `linkedin-cli-v*.zip` from [Releases](../../releases) and extract it to a folder on your PC.
...
| Command | Description |
|---------|------------|
| `linkedin-cli init` | Create local SQLite database |
...
Everything lives in `%LOCALAPPDATA%\LinkedInCLI\`:

| Path | Purpose |
|------|---------|
| `scanner.sqlite` | SQLite database (posts, scan queue, snapshots) |
| `config.json` | Your LinkedIn profile URL and scanner settings |
| `browser-profile/` | Chromium profile with your LinkedIn session |

Run `linkedin-cli reset --force --reinit` to delete all of it.
